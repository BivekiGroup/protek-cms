...
import { getNetworkCount, onNetworkChange } from '@/lib/network-activity'
...
export function RoutePreloader() {
  ...
  useEffect(() => {
    setNet(getNetworkCount())
    const unsub = onNetworkChange((c) => setNet(c))
    return () => { unsub() }
  }, [])

  if (!visible && net === 0) return null
