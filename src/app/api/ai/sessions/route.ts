...
export async function GET(req: NextRequest) {
  ...
  // Fallback raw
  const rows = await prisma.$queryRawUnsafe<any[]>(`SELECT id, title, model, "createdAt", "updatedAt" FROM "chat_sessions" WHERE "userId"='${user.userId.replace(/'/g, "''")}' ORDER BY "updatedAt" DESC LIMIT 100`)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (anyPrisma.chatSession?.create) {
      const created = await anyPrisma.chatSession.create({ data: { title, model, userId: user.userId } })
  ...
  const id = randomUUID()
  await prisma.$executeRawUnsafe(`INSERT INTO "chat_sessions" (id, "userId", title, model) VALUES ('${id}', '${user.userId.replace(/'/g, "''")}', '${title.replace(/'/g, "''")}', '${model.replace(/'/g, "''")}')`)
