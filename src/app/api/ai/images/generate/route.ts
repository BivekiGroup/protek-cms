...
export async function POST(req: NextRequest) {
  try {
    ...
    const filesUrl = Array.isArray(body?.filesUrl) ? body.filesUrl.filter((u: any) => typeof u === 'string') : undefined
    const filesBase64 = Array.isArray(body?.filesBase64) ? body.filesBase64.filter((u: any) => typeof u === 'string') : undefined
    const output_format = typeof body?.output_format === 'string' ? body.output_format : undefined
    const image_size = typeof body?.image_size === 'string' ? body.image_size : undefined
    ...
    if (filesUrl?.length) payload.filesUrl = filesUrl
    if (filesBase64?.length) payload.filesBase64 = filesBase64
    if (output_format) payload.output_format = output_format
    if (image_size) payload.image_size = image_size
    const res = await fetch(`${base}/images/generations`, { method: 'POST', headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
